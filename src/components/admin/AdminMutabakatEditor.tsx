...
export default function AdminMutabakatEditor() {
  ...
  const fetchMutabakat = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/admin/mutabakat');
      if (!response.ok) {
      ...
      showError('Mutabakat Zaptı içeriği yüklenirken bir hata oluştu');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to save mutabakat');
      }

      showSuccess('Mutabakat Zaptı içeriği başarıyla kaydedildi');
    } catch (error: any) {
        ...
        <>
          ...
          <div className="bg-white border border-black/10 p-6">
            ...
            <input
              ...
              placeholder="MUTABAKAT ZAPTI BAŞLIĞI"
            />
          </div>
